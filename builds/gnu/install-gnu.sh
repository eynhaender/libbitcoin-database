...
#
###############################################################################
# Script managing the build and installation of libbitcoin-database and its dependencies.
#
# Script options:
...
main()
{
    ...
    if ! [[ "${BUILD_SKIP_TESTS}" == "yes" ]]; then
        test_make "libbitcoin-database" "check" "${PARALLEL}"
    fi
...
help()
{
    msg "Script managing the build and installation of libbitcoin-database and its dependencies."
    msg ""
    msg "Script options:"
